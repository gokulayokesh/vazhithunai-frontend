...
class RegisterController extends Controller
{
    ...
    public function storeStep(Request $request)
    {
        ...
        try {
            ...
            $multiFields = ['interests', 'hobbies', 'favorite_cuisine', 'favorite_music', 'sports', 'languages_known'];

            foreach ($multiFields as $field) {
                if ($request->has($field)) {
                    $validated[$field] = is_array($request->$field)
            ...
            // ✅ Mark profile as completed after final step
            if (in_array($step, ['horoscope', 'images'])) {
                $user->profile_completed = 1;
                $user->profile_step = $step;
                $user->save();
            }else{
    ...
    public function getUserDetails()
    {
        ...
        if (!$details) {
            return response()->json(['status' => 404, 'message' => 'No details found'], 404);
        }
